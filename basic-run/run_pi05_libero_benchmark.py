"""
Simplified script to download and benchmark pi05-libero on LIBERO using existing openpi infrastructure.

This script leverages the existing openpi Policy and checkpoint loading infrastructure
to evaluate the pi05_libero checkpoint on LIBERO benchmarks.

Usage:
    # Run on libero_spatial with 10 episodes per task
    python run_pi05_libero_benchmark.py --task_suite libero_spatial --num_episodes 10

    # Run on libero_10 with 50 episodes
    python run_pi05_libero_benchmark.py --task_suite libero_10 --num_episodes 50

    # Use local checkpoint instead of downloading from GCS
...
def load_pi05_libero_policy(
    ...
    Args:
        checkpoint_dir: Path to checkpoint directory (can be GCS URL or local path)
    ...
    """
    print(f"Loading pi05-libero policy from: {checkpoint_dir}")
...
def run_benchmark(
    ...
    print("=" * 80)
    policy = load_pi05_libero_policy(checkpoint_dir)
...
def main():
    parser = argparse.ArgumentParser(
        ...
        epilog="""
Examples:
  # Benchmark on libero_spatial with 10 episodes per task
  python run_pi05_libero_benchmark.py --task_suite libero_spatial --num_episodes 10

  # Benchmark on libero_10 with 50 episodes per task
  python run_pi05_libero_benchmark.py --task_suite libero_10 --num_episodes 50
